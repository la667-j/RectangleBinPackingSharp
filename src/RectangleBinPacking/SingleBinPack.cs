...
namespace RectangleBinPacking
{
    ...
    public class SingleBinPack
    {
        ...
        private NestingSolution SolveSingleTypeHeightOptimized(List<PartItem> items)
        {
            ...
            if (maxRowH > 0)
            {
                ...
                if (wVert + hVert > sheetH)
                {
                    ...
                    if (IsBetterSolution(nestingSolution_V, nestingSolution_H))
                    {
                ...
                if (IsBetterSolution(nestingSolution_V, curBNestingSolution))
                {
                    curBNestingSolution = nestingSolution_V;
                }

                if (IsBetterSolution(nestingSolution_H, curBNestingSolution))
                {
                    curBNestingSolution = nestingSolution_H;
                }

                return curBNestingSolution;
            }
            ...
            int totalCount, double angleHorz, double angleVert)
        {
            ...
            Dictionary<double, List<Vector2d>> offsetsH = new();
            var maxX_H = 0;
            ...
            {
                var maxCol =
                    Math.Min((totalCount + maxRowV - 1) / maxRowV - 1,
                        sheetW / hVert - 1); // Calculate max columns - 1, special handling for last column (compare horizontal vs vertical, choose smaller maxX)
                ...
                if (maxRowH <= 0)
                {
                    ...
                    {
                        TryAddOffset(offsetsH, angleHorz,
                            new Vector2d(maxCol * hVert + offset_H.X, i * wVert + offset_H.Y));
                    }
                ...
                else
                {
                    ...
                    if (num1 > num2 || (num1 == num2 && maxX1 < maxX2))
                    {
                        ...
                        else
                        {
                            ...
                            int col1Real = (maxX1 - maxCol * hVert) / wVert;
                            int mod1 = num1 % (col1Real * maxRowH);
                            for (int i = 0; i < col1Real && num1 > 0; i++)
                            {
                                for (int j = 0; j < maxRowH && num1 > 0; j++)
                                {
                                    num1--;
                                    TryAddOffset(offsetsH, angleVert,
                                        new Vector2d(maxCol * hVert + i * wVert + offset_V.X, j * hVert + offset_V.Y));
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            num2 -= maxRowV;
                            int col2Real = (maxX2 - ((maxCol + 1) * hVert)) / wVert;
                            int mod2 = num2 % (col2Real * maxRowH);
                            for (int i = 0; i < col2Real && num2 > 0; i++)
                            {
                                for (int j = 0; j < maxRowH && num2 > 0; j++)
                                {
                                    num2--;
                                    TryAddOffset(offsetsH, angleVert,
                                        new Vector2d((maxCol + 1) * hVert + i * wVert + offset_V.X,
                                            j * hVert + offset_V.Y));
                                }
                            }
        ...
        {
            // 2. Try all vertical
            Dictionary<double, List<Vector2d>> offsetsV = new();
                ...
                if (newH > wVert)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            outerContourPoint_V.Update((colMax - 1) * hVert, colMax * hVert,
                                hStart + maxRowH_new * wVert);
                        }
                    }
                }
            }
          ...
          int totalCount, double angleHorz, double angleVert, bool optimize = false)
        {
          ...
          List<(int, int, int)> interval_V = new() { (0, 0, interval_V_Leng) }; // Vertical
          List<(int, int, int)> interval_H = new() { (0, interval_V_Leng, interval_H_Leng) }; // Horizontal

          // 2. Start packing
          int remainNum = totalCount;
          Dictionary<double, List<Vector2d>> offsets = new();
          int curMaxX = 0;
              ...
              else
              {
                ...
                while (remainNum > 0)
                {
                  ...
                  if (y3 == interval_H_Leng)
                  {
                    ...
                    interval_H = new() { (interval_H[0].Item1, interval_H[0].Item2, y3) };
                    remainNum--;
                    fillable_H = false;
                    break;
                  }
              ...
              else
              {
                ...
                while (remainNum > 0)
                {
                  ...
                  if (y4 == interval_V_Leng)
                  {
                    // Directly fill the gap
                    fillable_V = false;
                    interval_V = new() { (interval_V[0].Item1, interval_V[0].Item2, y4) };
                    remainNum--;
                    break;
                  }
              ...
              if (curMaxX_H != curMaxX_V)
              {
                ...
                if (num1 > 0 || num2 > 0)
                {
                  if (num1 > 0 && (num1 > num2 || (num1 == num2 && max1 < max2)))
                  {
                    ...
                    int col1Real = (max1 - interval_H[0].Item1) / wVert;
                    int row1Real = (sheetH - interval_V_Leng) / hVert;
                    int mod = num1 % (col1Real * row1Real); // Check for gaps
                    int num1Copy = num1;
                    ...
                    else
                    {
                      interval_H = new()
                      {
                        new(interval_H[0].Item1 + col1Real * wVert, interval_H[0].Item2, (row1Real - mod) * hVert),
                        new(interval_H[0].Item1, interval_H[0].Item2 + (row1Real - mod) * hVert, mod * hVert),
                      };
                    }
                  }
                  else if (num2 > 0 && (num2 > num1 || (num2 == num1 && max2 < max1)))
                  {
                    if (!merge2) // No vertical, only horizontal
                    {
                      ...
                      int col2Real = (max2 - interval_H[0].Item1) / hVert;
                      int row2Real = (sheetH - interval_V_Leng) / wVert;
                      int mod = num2 % (col2Real * row2Real); // Check for gaps
                      int num2Copy = num2;
                      ...
                      else
                      {
                        interval_H = new()
                        {
                          new(interval_H[0].Item1 + col2Real * hVert, interval_H[0].Item2, (row2Real - mod) * wVert),
                          new(interval_H[0].Item1, interval_H[0].Item2 + (row2Real - mod) * wVert, mod * wVert),
                        };
                      }
                    }
                    else // Mixed horizontal/vertical
                    {
                      ...
                      int num2ColMaxH =
                        Math.Min((numRemain + num2RowMaxH - 1) / num2RowMaxH, newWidth / hVert); // Theoretical max columns
                      ...
                      {
                        // Then place vertical
                        int col3Real = (max2 - num2ColMaxH * hVert - interval_H[0].Item1) / wVert; // Reverse calculate columns
                        int row3Real = (sheetH - interval_V_Leng) / hVert;
                        int num3Copy = num3;
                        int mod = num3 % (col3Real * row3Real); // Check for gaps
                        ...
                        else
                        {
                          interval_H = new()
                          {
                            new(interval_H[0].Item1 + col3Real * wVert, interval_H[0].Item2, (row3Real - mod) * hVert),
                            new(interval_H[0].Item1, interval_H[0].Item2 + (row3Real - mod) * hVert, mod * hVert),
                          };
                        }
                      }
                    }
              ...
              if (group_V == 1)
              {
                  ...
                  while (true) // Loop attempt
                  {
                        ...
                        else
                        {
                          ...
                          else
                          {
                            ...
                            interval_H = new()
                            {
                              ...
                              (interval_H[0].Item1, interval_H[0].Item2 + hVert, interval_H_Leng - hVert + reminTopY),
                            };
                          }
                        }
                      }
                    ...
                    else
                    {
                      if (is_has_remainY && curMaxX_V - wVert > curMaxX_H)
                      {
                        ...
                        {
                          interval_H = new() { (interval_H[0].Item1, interval_H[0].Item2, sheetH - interval_V_Leng) };
                        }
                        else
                        {
                          ...
                          interval_H = new()
                          {
                            ...
                            (interval_H[1].Item1, interval_H[1].Item2 + hVert, sheetH - interval_H[0].Item3 - hVert),
                          };
                        }
                      }
                      else
                ...
                else
                {
                  ...
                  if (numH > 0 && curMaxX_H < curMaxX_V - thresholdV)
                  {
                    ...
                    var lastColNum = interval_V[0].Item3 / hVert;
                    if (numH >= lastColNum)
                    {
                      // Move all
                      // Place all
                      ...
                      interval_H = new()
                      {
                        (interval_H[0].Item1, interval_H[0].Item2, interval_H[0].Item3 + lastColNum * wVert),
                    ...
                    else
                    {
                      ...
                      if (interval_V.Count == 1)
                      {
                        ...
                        interval_V = new()
                        {
                          ...
                          (interval_V[0].Item1 - wVert, interval_V[0].Item2 + interval_V_Leng - numH * hVert,
                            numH * hVert),
                        };
                      }
                      else
                      {
                        interval_V = new()
                        {
                          (interval_V[0].Item1, interval_V[0].Item2, interval_V[0].Item3 - numH * hVert),
                          (interval_V[1].Item1, interval_V[1].Item2 - numH * hVert, interval_V[1].Item3 + numH * hVert),
                        };
                      }

                      interval_H = new() { (interval_H[1].Item1, interval_H[0].Item2, sheetH - interval_V_Leng) };
                    }
              ...
              else
              {
                ...
                if (interval_H.Count == 1 || remainHY < hVert)
                {
                  ...
                  while (interval_H.Count > 1 && remainHY > wVert && curMaxX_V - curMaxX_H > thresholdV)
                  {
                    ...
                    if (interval_V.Count == 1) // Last vertical column is full
                    {
                      if (maxRowV <= rowHMax) // Place all in horizontal area
                      {
                        ...
                        interval_V = new() { (interval_V[0].Item1 - wVert, interval_V[0].Item2, interval_V_Leng) };
                      ...
                      else
                      {
                        ...
                        interval_V = new()
                        {
                          (interval_V[0].Item1, interval_V[0].Item2, interval_V_Leng - rowHMax * hVert),
                          (interval_V[0].Item1 - wVert, interval_V[0].Item2 + rowHMax * hVert, rowHMax * hVert),
                        };
                      }
                    }
                    else
                    {
                      ...
                      int lastVCol = interval_V[0].Item3 / hVert;
                      if (lastVCol <= rowHMax)
                      {
                  ...
                  optCols(ref interval_H, sheetH, interval_V_Leng, ref curMaxX_H, wVert, ref curMaxX_V,
                    offsets, angleVert, angleHorz, offset_H, offset_V, ref interval_V, hVert, interval_H_Leng);
                }
                else
                {
                  // Check if the last horizontal column can be vertical itself
                  int lastNum = interval_H[0].Item3 / wVert;
                  if (lastNum * hVert <= sheetH - interval_V_Leng)
                  {
                    ...
                    {
                      interval_H = new() { (interval_H[1].Item1 + wVert, yStart, sheetH - interval_V_Leng) };
                    }
                    else
                    {
                      interval_H = new()
                      {
                        ...
                        (interval_H[1].Item1, yStart + lastNum * hVert, sheetH - interval_V_Leng - lastNum * hVert),
                      };
                    }
                    ...
                    {
                      var reaminY1 = interval_H[1].Item3 / hVert; // How many more vertical ones can fit
                      if (interval_V.Count() == 1 && reaminY1 > 0) // Last vertical column is full
                      {
                        if (reaminY1 >= group_V) // Place all
                        {
                          ...
                          {
                            newVectorV.Add(new Vector2d(interval_H[1].Item1, interval_H[1].Item2 + i * hVert));
                          }

                          curMaxX_V -= wVert;
                          int lastVColNum = offsets[angleVert].Count;
                          offsets[angleVert] = offsets[angleVert].Take(lastVColNum - group_V).ToList();
                          interval_V = new() { (interval_V[0].Item1 - wVert, interval_V[0].Item2, interval_V_Leng) };
                          interval_H = new() { (interval_H[1].Item1, interval_H[0].Item2, sheetH - interval_V_Leng) };
                        }
                        else
                        {
                          ...
                          int lastVColNum = offsets[angleVert].Count;
                          offsets[angleVert] = offsets[angleVert].Take(lastVColNum - reaminY1).ToList();
                          interval_V = new()
                          {
                            (interval_V[0].Item1, interval_V[0].Item2, interval_V_Leng - reaminY1 * hVert),
                            (interval_V[0].Item1 - wVert, interval_V[0].Item2 + interval_V_Leng - reaminY1 * hVert,
                              reaminY1 * hVert),
                          };
                          interval_H = new() { (interval_H[1].Item1, interval_H[0].Item2, sheetH - interval_V_Leng) };
                        }
                      ...
                      else
                      {
                        if (reaminY1 > 0)
                        {
                          ...
                          var lastCol = interval_V[0].Item3 / hVert;
                          if (reaminY1 >= lastCol) // Place all
                          {
                            ...
                            curMaxX_V -= wVert;
                            int lastVColNum = offsets[angleVert].Count;
                            offsets[angleVert] = offsets[angleVert].Take(lastVColNum - lastCol).ToList();
                            interval_V = new() { (interval_V[0].Item1 - wVert, interval_V[0].Item2, interval_V_Leng) };
                            interval_H = new() { (interval_H[1].Item1, interval_H[0].Item2, sheetH - interval_V_Leng) };
                          }
                          else
                  ...
                  else
                  {
                    ...
                    var remainY = (interval_H_Leng - interval_H[0].Item3) / hVert; // Vertical quantity
                    var remainY2 = (interval_H_Leng - interval_H[0].Item3) / wVert; // Horizontal quantity
                    var lastCols = interval_V[0].Item3 / hVert; // Part count in last vertical column
                    if (remainY > 0 && remainY == remainY2)
                    {
                      ...
                      if (remainY >= lastCols)
                      {
                        ...
                        interval_H = new()
                        {
                          (interval_H[0].Item1, interval_H[0].Item2, interval_H[0].Item3 + lastCols * hVert),
                    ...
                    else if (remainY2 > 0 && remainY2 > remainY)
                    {
                      ...
                      if (remainY2 >= lastCols)
                      {
                        ...
                        interval_H = new()
                        {
                          (interval_H[0].Item1, interval_H[0].Item2, interval_H[0].Item3 + lastCols * wVert),
            ...
            else
            {
                  ...
                  && curMaxX_V > curMaxX_H && curMaxX_V - 2 * wVert < curMaxX_H)
              {
                ...
                if (newMaxX < oldMaxX)
                {
                  ...
                  offsets[angleVert].Remove(offsets[angleVert].Last());
                  TryAddOffset(offsets, angleHorz,
                    new Vector2d(interval_V[0].Item1 - 2 * wVert + offset_H.X, interval_V[0].Item2 + offset_H.Y));
                  TryAddOffset(offsets, angleVert,
                    new Vector2d(interval_H[0].Item1 + offset_V.X, interval_H[0].Item2 - (hVert - wVert) + offset_V.Y));
            ...
            if (interval_H.Count == 1)
            {
              if (curMaxX_H + wVert < curMaxX_V - thresholdV)
              {
                ...
                else
                {
                  ...
                  {
                    (interval_H[0].Item1 + wVert, interval_H[0].Item2, hVert),
                    (interval_H[0].Item1, interval_H[0].Item2 + hVert, sheetH - interval_V_Leng - hVert),
                  };
                }
              }
              else
